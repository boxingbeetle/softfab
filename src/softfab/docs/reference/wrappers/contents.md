...
SF\_USER\_INPUT_*
:    Actually this isn't a variable but a value. SF\_USER\_INPUT_* is used as the _`taskkey*`_ and _`task name*`_ in SF\_PROD in case the product was provided by the user as input to a task. Thus, this manual input product is not generated by any previous task. It gets a sequence number, starting at 0, for every unique user input.
