...
def testGogsPushType():
    """Check whether a push event is identified as such."""
    assert gogs.getEvent(emptyHeaders) == WebhookEvents.UNSUPPORTED
    assert gogs.getEvent(pushHeadersGogs) == WebhookEvents.PUSH
    assert gogs.getEvent(pushHeadersGogs.replace('X-Gogs-Event', 'beep')
                         ) == WebhookEvents.UNSUPPORTED

def testGogsPushVerifySignature():
    """Check signature."""
    assert not gogs.verifySignature(emptyHeaders, pushBodyGogs, b'12345')
    assert not gogs.verifySignature(pushHeadersGogs, pushBodyGogs, b'12345')
    assert gogs.verifySignature(pushHeadersGogs, pushBodyGogs, b'letmein')

def testGogsPushFindRepo():
...
def testGitHubPushType():
    """Check whether a push event is identified as such."""
    assert github.getEvent(emptyHeaders) == WebhookEvents.UNSUPPORTED
    assert github.getEvent(pushHeadersGitHub) == WebhookEvents.PUSH
    assert github.getEvent(pushHeadersGitHub.replace('X-GitHub-Event', 'beep')
                           ) == WebhookEvents.UNSUPPORTED

def testGitHubPushVerifySignature():
    """Check signature."""
    assert not github.verifySignature(emptyHeaders, pushBodyGitHub, b'12345')
    assert not github.verifySignature(pushHeadersGitHub, pushBodyGitHub, b'12345')
    assert github.verifySignature(pushHeadersGitHub, pushBodyGitHub, b'letmein')

def testGitHubPushFindRepo():
    """Find repository URLs in JSON body."""
...
def testGitHubPushFindBranches():
    """Find branch name in JSON body."""
    payload = json.loads(pushBodyGitHub)
